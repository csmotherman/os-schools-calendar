...
as $$
  select public.is_admin(check_user_id)
    or exists (
      ...
      where p.id = check_user_id
        and p.role = 'PROGRAM_USER'
        and p.account_status = 'APPROVED'
        and pm.status = 'APPROVED'
        and pm.program_id = check_program_id
        and pr.active = true
    );
$$;
...
create trigger calendars_validate_dates
before insert or update of school_year_id, start_date, end_date on public.calendars
for each row execute function public.validate_calendar_dates();
...
create trigger calendar_days_validate_date
before insert or update of calendar_id, date on public.calendar_days
for each row execute function public.validate_calendar_day_date();
...
create trigger calendar_day_activities_validate_compatibility
before insert or update of calendar_day_id, activity_type_id on public.calendar_day_activities
for each row execute function public.validate_activity_compatibility();
...
create trigger calendar_days_validate_session_change
before update of in_session on public.calendar_days
for each row execute function public.validate_session_change();
...
create trigger calendar_days_enforce_blocked_date
before insert or update of calendar_id, date, in_session on public.calendar_days
for each row execute function public.enforce_blocked_date();
...
create trigger calendar_day_activities_enforce_blocked_date
before insert or update of calendar_day_id on public.calendar_day_activities
for each row execute function public.enforce_blocked_date_activity();
