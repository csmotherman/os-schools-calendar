...
-- Stable fixture identities. Everything rolls back at the end of this test.
...
-- A valid A calendar will have two session days; B will have only one.
...
-- Program A user access and controlled creation.
...
-- Capture IDs outside RLS so later tests can deliberately attack them.
reset role;
...
select is(
  (
    ...
    where id = (select id from test_ids where key = 'a_day_1')
  ),
  'saved through controlled RPC',
...
-- Program B must not be able to observe or mutate Program A.
...
-- Pending account has identity but no program data access.
...
-- Blocking requirement is enforced by PostgreSQL, not just by the UI.
select set_config('request.jwt.claim.sub', '10000000-0000-0000-0000-000000000003', true);
select throws_ok(
  $$
...
-- Valid A calendar can submit, then becomes immutable for its program user.
select set_config('request.jwt.claim.sub', '10000000-0000-0000-0000-000000000002', true);
select lives_ok(
...
-- Approved administrator can review across programs.
...
-- Any later program edit reopens review.
select set_config('request.jwt.claim.sub', '10000000-0000-0000-0000-000000000002', true);
select lives_ok(
  $$
    select public.save_calendar_day(
